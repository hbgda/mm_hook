...
scan_func_static!(crate::patterns::ASSETS_LOADASSET, LOAD_ASSET(*const (), u64, u64, *const u8, u64, u64, u64) -> *const ());
pub unsafe fn load_asset(asset_manager: *const (), hash: u64) -> *const () {
    LOAD_ASSET(asset_manager, hash, 0, std::ptr::null(), 0, 0, 0)
}

pub unsafe fn load_asset_ex(asset_manager: *const (), hash: u64, u1: u64, path: *const u8, u3: u64, u4: u64, u5: u64) -> *const () {
    LOAD_ASSET(asset_manager, hash, u1, path, u3, u4, u5)
}